...
- gateway proxy에서 JWT를 공통 검증하고 claim header를 application으로 전달할지, namespace 영역의 waypoint proxy에서 service 단위로 검증할지 결정이 필요하다.
- gateway proxy에서 공통 검증하는 경우 request authentication policy는 platform 공통 정책에 가까워진다.
- waypoint proxy에서 service 단위로 검증하는 경우 request authentication policy는 application chart에서 관리하는 편이 자연스럽다.
...
- JWT request authentication을 gateway proxy에서 공통 처리할지, namespace waypoint proxy에서 service 단위로 처리할지
