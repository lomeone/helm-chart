...
- stateless application은 내부 구현에서 Kubernetes `Deployment`, Argo Rollouts `Rollout`, 또는 다른 progressive delivery controller 리소스로 렌더링될 수 있다.
- stateful application은 내부 구현에서 Kubernetes `StatefulSet` 중심으로 렌더링된다.
- Kubernetes `ReplicaSet`은 보통 `Deployment`가 관리하는 하위 리소스이므로, chart 사용자-facing workload type으로 직접 노출하지 않는다.
...
- `workload.type: stateless`의 기본 구현을 Kubernetes `Deployment`로 할지, Argo Rollouts `Rollout`으로 할지 결정이 필요하다.
- canary/blue-green 전략을 선택한 경우에는 해당 전략을 지원하는 controller가 필요하다.

### 10.5 Security Context Placement
...
- implementation-specific authentication policy
...
Inbound authentication/authorization policy 지원 항목:

- JWT validation 사용 여부
- issuer
- audiences
- JWKS URI 또는 JWKS 참조
- claim to request header mapping
...
- values에서는 Istio `RequestAuthentication`/`AuthorizationPolicy` 같은 구현체 이름을 직접 노출하지 않는다.
- 현재 인프라에서는 inbound authentication/authorization policy가 Istio 리소스로 렌더링될 수 있다.
...
이 정책은 application마다 route, method, claim 조건이 달라질 수 있으므로 `network` 섹션에서 application-level L7 policy로 관리한다.
...
- 인증 정책과 인가 정책을 chart에서 모두 생성할지, 인증 정책 일부를 namespace/platform 공통 정책으로 둘지는 추가 논의한다.
...
- OpenTelemetry `Instrumentation` 리소스는 namespace 또는 platform 공통 리소스로 관리하는 방식을 기본으로 본다.
- application chart는 기본적으로 `Instrumentation` CR을 생성하지 않고, 필요한 Pod annotation/env를 통해 injection을 활성화한다.

추후 다음 요구가 명확해지면 `observability` top-level 섹션을 재검토한다.

- `ServiceMonitor`/`PodMonitor`를 chart가 직접 생성해야 하는 경우
- application별 `Instrumentation` CR을 chart가 직접 생성해야 하는 경우
...
└── application-base
    ...
    │   ├── hpa.yaml
    │   ├── pdb.yaml
    │   ├── inbound-authentication-policy.yaml
...
- Progressive delivery controller version
- Gateway API version
...
- Gateway API와 progressive delivery controller CRD 버전은 실제 클러스터 설치 버전에 맞춘다.
...
- Kubernetes 최소 버전
- Helm 최소 버전
- stateless workload의 기본 내부 구현을 Kubernetes `Deployment`로 할지, progressive delivery controller 리소스로 할지
- progressive delivery controller를 어떤 구현체로 선택할지
...
- application별 OpenTelemetry `Instrumentation` 리소스 생성을 optional로 지원할지
- inbound authentication policy를 application chart에서 항상 생성할지, namespace/platform 공통 정책으로 둘지
